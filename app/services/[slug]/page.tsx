  ...
  return (
    <main className="sv-root">
      ...
      <div className="sv-wrap">
        ...
        <section className={`sv-hero ${s.motion ? "sv-hero--motion" : ""}`}><div className="sv-hero-copy"><div className="sv-kicker">{s.motion ? "Motion-led architecture" : "Service"}</div><h1>{s.h1}</h1><p className="sv-tagline">{s.tagline}</p><p className="sv-lead">{s.intro}</p></div><figure className="sv-proof">{s.motion ? <video autoPlay muted loop playsInline preload="metadata" poster={s.motion.poster} aria-label={s.motion.label}><source src={s.motion.src} type="video/mp4" /></video> : <img src={s.image} alt={s.imageAlt} width="1600" height="1200" fetchPriority="high" />}<figcaption>{s.motion ? `Playing now / ${s.motion.label}` : `Selected Artimist work / ${s.name}`}</figcaption></figure></section>
