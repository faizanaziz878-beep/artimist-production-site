  ...
  return (
    <main className={`visual-journey visual-journey-${kind}`}>
      <section className="vj-hero">
        {heroScene && <img src={heroScene.src} alt="" fetchPriority="high" decoding="async" />}
        ...
        <a className="vj-enter" href={`#${chapters[0]?.slug}`}>Enter the sequence <UiIcon className="vj-down-icon" name="chevron" size={15} /></a>
      </section>
      ...
      <nav className="vj-chapter-nav" aria-label="Portfolio chapters">
        {chapters.map((chapter, index) => <a href={`#${chapter.slug}`} key={chapter.slug}><small>{String(index + 1).padStart(2, "0")}</small><span>{chapter.title}</span></a>)}
        {technicalBoards.length > 0 && <a href="#technical-plates"><small>T</small><span>Technical plates</span></a>}
      </nav>
              ...
              return (
                <figure className={`vj-scene ${sceneIndex % 3 === 1 ? "is-inset" : ""} ${sceneIndex % 3 === 2 ? "is-offset" : ""}`} key={`${chapter.slug}-${sceneIndex}-${scene.src}`}>
                  <div className="vj-media-stage">
                    ...
                    <img className="vj-media-primary" src={scene.src} alt={`${chapter.title} — ${scene.label}`} loading="lazy" decoding="async" onError={(event) => event.currentTarget.parentElement?.classList.add("is-missing")} />
                  </div>
      ...
      {technicalBoards.length > 0 && (
        <section className="vj-technical" id="technical-plates">
          <header><span>T / {String(technicalBoards.length).padStart(2, "0")}</span><h2>Drawings shown whole.</h2><p>Scroll sideways through the complete presentation plates. Every title, diagram, plan, section and annotation remains inside the frame.</p></header>
          <div className="vj-board-rail">
            {technicalBoards.map((board, index) => <figure key={board.src}><img src={board.src} alt={`${board.label} — ${board.detail}`} loading="lazy" decoding="async" /><figcaption><span>{String(index + 1).padStart(2, "0")} / {String(technicalBoards.length).padStart(2, "0")}</span><strong>{board.label}</strong><small>{board.detail}</small></figcaption></figure>)}
          </div>
        </section>
      )}

      <section className="vj-end">
        <span>End of sequence</span><h2>See the work.<br /><em>Feel the space.</em></h2><div><Link href="/case-studies">Read the case studies <UiIcon name="arrow" size={16} /></Link><Link href="/contact">Start a project <UiIcon name="arrow" size={16} /></Link></div>
      </section>
    </main>
  );
}
