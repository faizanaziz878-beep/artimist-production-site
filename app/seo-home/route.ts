...
function enhance(source: string) {
  ...
  html = html.replace('</head>', `<meta name="google-site-verification" content="${GOOGLE_VERIFICATION}"><meta name="robots" content="index,follow,max-image-preview:large,max-snippet:-1,max-video-preview:-1">${googleTag}${seoStyle}<link rel="stylesheet" href="/home-typography.css">${webpageSchema}</head>`);
